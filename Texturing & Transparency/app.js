...
function createShaders(loadedAssets) {
    ...
    textureShaderProgram.uniforms = {
        ...
        scrollUniform: gl.getUniformLocation(textureShaderProgram, "uscroll")
    };
}
...
function updateAndRender() {
    ...
    // todo #10 apply the painter's algorithm
        //furthest sphere from the camera draw it first
        const objects =sphereGeometryList.sort(function(a,b){
            const first = (camera.getPosition().subtract(a.getPosition().length()));
            const second = (camera.getPosition().subtract(b.getPosition().length()));
            if(first>second){
                return -1; 
            }else{
                return 1; 
