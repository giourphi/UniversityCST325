...
void main(void) {
    ...
    // 1. normalize the light direction and store in a separate variable
      vec3 light = normalize(uLightDirection);
         vec3 lightposition  = normalize(uLightDirection -vWorldPosition);
    ...
    float cosine_angle = dot(world, lightposition);
    ...
    // 2. in world space, calculate the reflection vector (normalized)
      vec3 reflection = normalize(-lightposition + dot(lightposition,world)*world*2.0);
