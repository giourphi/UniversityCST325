...
function createShaders(loadedAssets) {
    ...
    flatcolorshader = createCompiledAndLinkedShaderProgram(loadedAssets.flatTextVS,loadedAssets.flatTextFS);

    flatcolorshader.attributes = {
        vertexPositionAttribute: gl.getAttribLocation(flatcolorshader,"aVertexPosition"),
    };

    flatcolorshader.uniforms = {
        worldMatrixUniform: gl.getUniformLocation(flatcolorshader, "uMVMatrix"),
        viewMatrixUniform: gl.getUniformLocation(flatcolorshader, "uPMatrix"),
        projectionMatrixUniform: gl.getUniformLocation(flatcolorshader, "uProjection")
    };
...
function createScene() {
    ...
    whitesphere = new WebGLGeometryJSON(gl,flatcolorshader);
    whitesphere.create(loadedAssets.sphereJSON,loadedAssets.marbleImage);

    var scalewhitesphere = new Matrix4().makeScale(.003,.003,.003,.003);
    var whitespheretranslation = new Matrix4().makeTranslation(5,3,0,0)
    whitesphere.worldMatrix.makeIdentity();
    whitesphere.worldMatrix.multiply(whitespheretranslation).multiply(scalewhitesphere);
...
function updateAndRender() {
    ...
    // add keyboard controls for changing light direction here
        ...
        //rotating to the up    
        var rotationright  = new Matrix4().makeTranslation(0,1.5,0);
        rotationright.makeRotationY(Math.sin(-45));
        lightDirection =rotationright.multiplyVector(lightDirection);


        whitesphere.worldMatrix= rotationright.multiply(whitesphere.worldMatrix);
    ...
    gl.useProgram(flatcolorshader);
    var uniforms = flatcolorshader.uniforms;
    gl.uniform3f(uniforms.lightDirectionUniform,lightDirection.x,lightDirection.y,lightDirection.z);
    gl.uniform3f(uniforms.cameraPositionUniform,cameraPosition.x,cameraPosition.y,cameraPosition.z);
    ...
    whitesphere.render(camera,projectionMatrix , flatcolorshader);
}
